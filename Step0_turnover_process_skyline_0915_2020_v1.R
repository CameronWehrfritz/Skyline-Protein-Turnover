...
#------------------------------------------------------------------------------------
# read in pre-processed data file: "df_processed_ca6_v4_no_dups_2020_0701_v1.csv"
#df <- read.csv("df_processed_ca6_v4_no_dups_2020_0701_v1.csv", stringsAsFactors = FALSE)
#------------------------------------------------------------------------------------
...
# all proteins
proteins <- unique(df$Protein.Gene)

# test case - just one protein for testing
proteins <- unique(df$Protein.Gene)[100]
...
#------------------------------------------------------------------------------------
# 7/27/2020
# df.solutions <-  read.csv("//bigrock/GibsonLab/users/Cameron/2020_0723_OCR1_turnover/Results/step0/output_data/df_solutions_fulltest_run1_0727_2020_v1.csv", stringsAsFactors = FALSE)
...
df.solutions <- df.solutions %>%
  ...
  dplyr::mutate(Condition=paste0(Cohort, "_D", Timepoint)) %>% # create new Condition column
...
# 0 being no filter which inclues all data. 
...
## We may want this to be a user defined value instead: between 0 and 100 (including 0, not including 100)
IDP.threshold <- 0.0 # may set this value to anything between 0 and 100. 0 being no filter which inclues all data. 70 or 80 is typically a good value which retains a large amount of the data.
...
# # make sure Precursor Pool is numeric
# df.precursor.pool <- df.precursor.pool %>%
#   mutate_at(vars(Precursor.Pool), funs(as.numeric(.)))

# add up all Areas A0 to A4
df.precursor.pool <- df.precursor.pool %>%
...
# average turnover score filters: 1, 0.8, 0.6, 0.4, 0.2
ATS <- 0.2
...
density.percnew <- df.pp.ats.filtered %>%
  mutate(Condition= fct_relevel(Condition, "D3_C", "D3_KO", "D7_C", "D7_KO", "D15_C", "D15_KO", "D30_C", "D30_KO")) %>%
  ggplot(aes(x=Perc.New.Synth, fill=Condition)) + 
  geom_histogram(aes(y=..density..), alpha=0.2, col="black", position='identity') +
  geom_density(alpha=0.2) +
  #geom_vline(data=df.pp.medians, aes(xintercept=Precursor.Pool, col=Condition), linetype="dashed", show.legend=FALSE) +
  facet_wrap(~ Condition, ncol = 2, scales = "fixed") +
  labs(title="Percent Newly Synthesized Distrubtion by Condition - no IDP filter and ATS<0.2", x="Percent Newly Synthesized", y="Density") +
  theme_bw() 
#------------------------------------------------------------------------------------
